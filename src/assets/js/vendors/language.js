// Language switch by country - Updated to use i18n

function changeLanguageFromDropdown(language) {
	...
	// Wait for i18n to be available, then use it
	if (typeof window.changeLanguage === 'function') {
		window.changeLanguage(langCode);
	} else {
		// If i18n not ready, wait a bit and try again
		setTimeout(() => {
			if (typeof window.changeLanguage === 'function') {
				window.changeLanguage(langCode);
			} else {
...
// Helper function to update language icon
...
// Make the dropdown function globally available
